...
void Deque::push_front(int element){
  ...
  if (first_element < 0){ // if the first element has gone past the front of the current block
    first_block--; 
    first_element = blocksize - 1; // make the first_element = to the last slot in the blocksize

    if (first_block < 0){ // if we've run out of room in the front
      growFront(); // double the map size from the front
    }

    if (blockmap[first_block] == nullptr){ // if the new block hasn't been allocated just like push_back
